...
int main() {
...

    {
        ...
        if (ok) {
            ...
            ok = (std::abs(*cc) < kTolFlat) && (std::abs(*pk) < kTolFlat) &&
                 (std::abs(*yz) < kTolFlat);
        }
...

    {
        ...
        std::printf(
            "--- 2. GBM (mu=0, sigma=0.02), n=10000, window=%zu (20%% band [%.4f, %.4f]) ---\n",
        ...
        bool ok = cc.has_value() && pk.has_value() && yz.has_value() && in_band(*cc) &&
                  in_band(*pk) &&
...

    {
        ...
        std::printf(
            "--- 3. Window efficiency (same synthetic series, GBM seed=42, OHLC seed=99) ---\n");
        ...
        for (std::size_t w : windows) {
            ...
            const auto pk =
                est.parkinson(std::span<const OhlcBar>(hist.data() + (hist.size() - w), w));
        ...
        const double pk20 =
            ...
            e20p.parkinson(std::span<const OhlcBar>(hist.data() + (hist.size() - 20), 20))
                .value_or(999.0);
        ...
        std::printf("    Point estimates at w=20: CC=%.6f  PK=%.6f  YZ=%.6f (YZ often lowest σ-hat "
                    "with range info)\n",
        ...
        const double pk200 =
            ...
            e200.parkinson(std::span<const OhlcBar>(hist.data() + (hist.size() - 200), 200))
                .value_or(-1.0);
        ...
        const bool sane = cc200 >= lo3 && cc200 <= hi3 && pk200 >= lo3 && pk200 <= hi3 &&
                          yz200 >= lo3 &&
        ...
        if (!sane) {
            ...
            pass("3. Efficiency table printed; at w=200 all estimators near 0.02 (YZ often best at "
                 "small w in "
...

    {
        ...
        auto eval_at = [&](std::size_t end_exclusive) -> std::tuple<double, double, double> {
            std::vector<Price> sub(closes.begin(),
                                   closes.begin() + static_cast<std::ptrdiff_t>(end_exclusive));

            std::vector<OhlcBar> hsub(hist.begin(),
                                      hist.begin() + static_cast<std::ptrdiff_t>(end_exclusive));
            ...
            const double p =
                ...
                e.parkinson(std::span<const OhlcBar>(hsub.data() + (hsub.size() - w), w))
                    .value_or(-1.0);
